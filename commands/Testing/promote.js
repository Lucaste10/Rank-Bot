...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option =>
			option.setName('username')
				.setDescription('The full roblox username of the player you would like to promote.')
				.setRequired(true))
		.addStringOption(option =>
			option.setName('reason')
				.setDescription('An optional reason to why you would like to promote the user')
				.setRequired(false)),
	async execute(interaction) {
		...
		const promotionEmbed = new EmbedBuilder()
			.setColor(0x79FF77)
			.setTitle(`Successfully promoted ${username}`)
